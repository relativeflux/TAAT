...
The TAAT analysis engine works by first creating a timbral profile of its input audio files. Each file is processed in chunks, the size of which can be determined by the user. The chunks are then compared using cross similarity matrices and [recurrence quantification analysis](https://en.wikipedia.org/wiki/Recurrence_quantification_analysis), in order to establish a similarity score. Sequences of chunk scores represent similar timbral profiles that might inform a consistent musical progression. Results can then be output to the terminal, plotted as charts, or saved to a `JSON` file which can be interpreted further by other applications or programs.
